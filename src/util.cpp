...
const char * const BITCOIN_CONF_FILENAME = "ultranatum.conf";
const char * const BITCOIN_PID_FILENAME = "ultranatumd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ultranatum" is a composite category enabling all Ultranatum-related debug output
            if(ptrCategory->count(string("ultranatum"))) {
...
#else
    const char* pszModule = "ultranatum";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UltranatumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UltranatumCore
    // Mac: ~/Library/Application Support/UltranatumCore
    // Unix: ~/.ultranatumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UltranatumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UltranatumCore";
#else
    // Unix
    return pathRet / ".ultranatumcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ultranatum.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ultranatum.conf
